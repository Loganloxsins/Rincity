...
data<-read.csv("./data09.csv",header=TRUE, sep=",",fileEncoding = "GBK")
#查看数据特征
dim(data) 
names(data) 
...
stab6<-clusterboot(data1,B=100,bootmethod="boot", 
                   clustermethod= kmeansCBI, krange=6, seed=7)
print(stab6)
...
names(data)[8]<-"kmcluster"
# 查看data数据
head(data)

write.csv(data, file= "D:/R/handbook/homework09/cluster1.csv")
...
maxmin <- data.frame(HEAT_ISLAND=c(0, 1),  GDP=c(1, 0), NPP=c(1,0), CO2_EMISSION=c(0,1))
maxmin
...
radarfig
# 设置轴标签
colnames(radarfig) <- c("热岛强度", "国内生产总值", "植物净初生产力","碳排放")
# 设置雷达图标注字体、字号、图例样式
# 参数pty、seg、pcol、plty、plwd、cglty、cglcol、vlcex用于设置雷达图样式
# 参数 centerzero表示绘制刻度的格式
radarchart(radarfig,pty = 32, axistype=0, axislabcol="black", seg=5, 
           pcol = c('blue','yellow','red','green','black','purple'), 
           plty = 1, plwd = 2, cglty = 1, cglcol = "black", 
           centerzero = TRUE, vlcex = 1, title ="kmeans聚类分析")
#绘制图例
legend("topleft", c("第一类","第二类","第三类","第四类","第五类","第六类"),
       fill = c('blue','yellow','red','green','black','purple'),cex = 1)

#.packages("eclust")
library(eclust)

#层次聚类，"hclust"表示聚类方式选择“层次聚类”
res.hc<- eclust(data1, k=5,"hclust")
...
stab<-clusterboot(ddata,B=100,bootmethod="boot", clustermethod=hclustCBI, k=5, seed=7,method="ward")
# 打印结果
print(stab)
